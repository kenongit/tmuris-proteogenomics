...
# ---------------------------------------------------------------------------
# Bede's version... Simply adds 'DECOY_' prefix after '>' of header
# Affects reverse decoys only (default behaviour)
# USAGE: decoy.pl input.fa output.fa
# ---------------------------------------------------------------------------
  ...
  # remove any non-residue characters from input sequence
    ...
    # chop into little pieces for output
      ...
      if ($keep_accessions) {
        ...
        print OUTFILE ">DECOY_" . $accession . " " . $description . $eol;
      }
